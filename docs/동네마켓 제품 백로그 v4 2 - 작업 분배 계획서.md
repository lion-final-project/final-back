...
| --- | --- | --- | --- |
| **Dev1(남혜린)** | 백엔드 리드 | Auth, Users, 공통 인프라 | Spring Security, JWT, OAuth2 |
| **Dev2(성시영)** | 주문/결제 담당 | Orders, Payments, Cart | PG 연동(토스페이먼츠), 트랜잭션 관리 |
| **Dev3(김한울)** | 마트 운영 담당 | Stores, Products, Store Ops | JPA, 파일 업로드(S3), 통계 쿼리 |
| **Dev4(백종현)** | 배달 시스템 담당 | Deliveries, Rider, 실시간 | WebSocket/STOMP, PostGIS, Redis |
| **Dev5(김한성)** | 구독/정산 담당 | Subscriptions, Settlements | 스케줄러(Spring Batch), 정산 로직 |
| **Dev6(김도은)** | 관리자 담당 | Admin, Approvals, Reports | 어드민 CRUD, 승인 워크플로우 |
| **Dev7(이해인)** | 리뷰/알림/기타 | Reviews, Notifications, Support, Misc | JPA, 알림 시스템, 콘텐츠 관리 |
...
| 담당 | 배정 US | SP | 비고 |
| --- | --- | --- | --- |
| **Dev1** | US-C01(5), US-C02(5) | **10** | Spring Security 설정, JWT 인증 |
| **Dev2** | US-C05(3), US-C14(3), US-C15(3) | **9** | 장바구니 CRUD + 1:1 문의 |
| **Dev3** | US-S01(5), US-S02(5) | **10** | 마트 입점 신청 + 상품 관리 CRUD |
| **Dev4** | US-R01(5), US-R02(3) | **8** | 라이더 등록 신청+ 상태 변경 |
| **Dev5** | US-A08(3), US-A11(3) + 인프라 | **6** | 공지사항 + FAQ CRUD + DB 스키마 생성 (ERD Layer 0~2), Redis Geo/PostGIS 설정 |
| **Dev6** | US-A01(5) | **8** | 마트 승인 워크플로우  |
...
| 담당 | 배정 US | SP | 비고 |
| --- | --- | --- | --- |
| **Dev1** | US-C03(8), US-A13(3) | **11** | 1:1 문의 답변,소셜 로그인 |
| **Dev2** | US-C06(13) | **13** | PG 결제 연동, 멀티스토어 주문 |
| **Dev3** | US-S03(3), US-S04(8) | **11** | 재고 관리 + 주문 접수/배차 |
| **Dev4** | US-R03(5), US-R04(3), US-R05(3) | **11** | 배달 수락 → 픽업 → 배송 시작, PostGIS 지역 검색 (용역) |
...
| 담당 | 배정 US | SP | 비고 |
| --- | --- | --- | --- |
| **Dev1** | US-C12(3), US-C13(5), US-C11(3) | **11** | 배송지 CRUD + 회원 탈퇴 |
| **Dev2** | US-C07(5) | **5** | 주문 취소 + PG 환불 연동 + 결제수단 관리 |
