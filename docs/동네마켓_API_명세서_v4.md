...
| **문서 버전** | v4.0 |
| **작성일** | 2026-01-30 |
| **Base URL** | `https://api.dongnaemarket.com/api` |
...
**토큰 전달 방식 (현재 구현)**  
- 회원가입·로그인·토큰 갱신 시: **HttpOnly Cookie**로 전달 (응답 본문에 토큰 미포함).  
- 쿠키 이름: `nm_accessToken`, `nm_refreshToken`.  
- 인증 필요 API 요청 시: `Authorization: Bearer {access_token}` 헤더 또는 동일 도메인에서 `nm_accessToken` 쿠키로 전달.
...
> **구현 참고**: 현재 백엔드에서는 모듈별 코드(`COMMON-xxx`, `AUTH-xxx` 등)를 사용한다. 인증 모듈 에러는 각 API 상세의 "에러 응답" 표를 참고한다.
...
**구현 참고 (협업용)**  
- JWT Access/Refresh 토큰: **HttpOnly Cookie**로 전달 (응답 본문에는 미포함). 쿠키 이름: `nm_accessToken`, `nm_refreshToken`.
- Bearer 인증: `Authorization: Bearer {access_token}` 또는 동일 도메인 요청 시 `nm_accessToken` 쿠키로 전달 가능.
- 토큰 갱신(API-AUTH-007): Request Body가 아닌 **Cookie의 refreshToken**으로 요청.

---

### API-AUTH-000: 이메일 중복 확인
...
| 필드 | 타입 | 설명 |
| --- | --- | --- |
| `duplicated` | Boolean | true: 이미 사용 중, false: 사용 가능 |

---

### API-AUTH-000-2: 휴대폰 중복 확인

| 항목 | 내용 |
| --- | --- |
| **Method** | `GET` |
| **URL** | `/api/auth/check-phone?phone={phone}` |
| **인증** | Public |
| **권한** | - |
| **설명** | 회원가입 전 휴대폰 번호 중복 여부 확인. |

**Query Parameter**

| 파라미터 | 타입 | 필수 | 설명 |
| --- | --- | --- | --- |
| `phone` | String | O | 휴대폰 번호 (010xxxxxxxx 형식) |
...
- **AccessToken/RefreshToken**: 응답 본문이 아닌 **Set-Cookie** 헤더로 전달 (HttpOnly, 쿠키 이름: `nm_accessToken`, `nm_refreshToken`).
- 본문 `data`에는 `userId`, `email`, `name`, `roles`만 포함.
...
| HTTP | 에러 코드 | 상황 |
| --- | --- | --- |
| `409` | `AUTH-002` / `AUTH-003` | 이메일 또는 휴대폰 번호 중복 |
| `400` | `COMMON-001` | 필수 필드 누락, 비밀번호 형식 불일치 |
| `422` | `AUTH-004` ~ `AUTH-006` | 필수 약관 미동의, 휴대폰 인증 미완료 |

> 실제 구현 에러 코드: `COMMON-xxx`, `AUTH-xxx` 형식 사용 (코딩 컨벤션).
...
### [필요] API-AUTH-003-2: 소셜 신규 가입 완료 (social-register)

| 항목 | 내용 |
| --- | --- |
| **Method** | `POST` |
| **URL** | `/api/auth/social-register` |
| **인증** | Public (세션 또는 onboarding 토큰 등 구현 방식에 따름) |
| **권한** | - |
| **설명** | 신규 소셜 유저가 필수 가입 요소(이메일 중복 확인, 휴대폰 인증, 주소, 약관 동의)를 완료한 후 호출. 회원 생성 및 JWT 발급. |
| **관련 FR** | FR-USR-002 |
| **비고** | 현재 구현은 `POST /api/auth/social-signup/complete` 로 동일 목적 제공. 토큰 기반 API로 통일 시 본 명세 적용. |

**Request Body (예시)** (주소 미수집)
...
```json
{
  ...
  "name": "홍길동",
  "phone": "01012345678",
  "phoneVerificationToken": "발급받은_휴대폰인증토큰",
  "termsAgreed": true,
  "privacyAgreed": true
}
```
...
| HTTP | 에러 코드 | 상황 |
| --- | --- | --- |
| `400` | `COMMON-001` 등 | 세션/토큰 없음 또는 이미 가입된 provider_user_id |
| `409` | `AUTH-002` | 이메일 중복 |
| `409` | `AUTH-003` | 휴대폰 중복 |
| `422` | `AUTH-004`~`AUTH-006` | 휴대폰 인증 토큰 없음/만료 |
...
| HTTP | 에러 코드 | 상황 |
| --- | --- | --- |
| `409` | `AUTH-003` | 이미 등록된 휴대폰 번호 |
| `429` | `AUTH-007` | 재발송 횟수 초과 (5회) |
...
| HTTP | 에러 코드 | 상황 |
| --- | --- | --- |
| `400` | `AUTH-009` | 인증번호 불일치 |
| `422` | `AUTH-006` | 인증번호 만료 (3분 초과) |
...
| **인증** | Public |
| **권한** | - |
| **설명** | Refresh Token으로 새 Access Token·Refresh Token을 발급받는다. |
| **관련 FR** | FR-USR-001 |
| **관련 UC** | UC-C02 |

**Request**

- **Request Body 사용 안 함.** Refresh Token은 **Cookie**로 전달 (`nm_refreshToken`).
- 클라이언트: `credentials: 'include'`로 동일 도메인(또는 CORS 허용) 요청 시 쿠키 자동 포함.

**Response (200 OK)**

- **AccessToken/RefreshToken**: 응답 본문이 아닌 **Set-Cookie** 헤더로 전달 (HttpOnly, `nm_accessToken`, `nm_refreshToken`).
- 본문 `data`는 `null` 또는 `"토큰이 갱신되었습니다."` 메시지.
...
| HTTP | 에러 코드 | 상황 |
| --- | --- | --- |
| `401` | `AUTH-010` | Refresh Token 만료/무효 |
...
| --- | --- |
| **Method** | `GET` |
| **URL** | `/api/users/me/payment-methods` |
| **인증** | Bearer Token |
| **권한** | 모든 인증 사용자 |
...
| --- | --- |
| **Method** | `DELETE` |
| **URL** | `/api/users/me/payment-methods/{paymentMethodId}` |
| **인증** | Bearer Token |
| **권한** | 모든 인증 사용자 |
...
| --- | --- |
| **Method** | `GET` |
| **URL** | `/api/stores/{storeId}/subscription-products` |
| **인증** | Bearer Token |
| **권한** | CUSTOMER |
...
| --- | --- |
| **Method** | `GET` |
| **URL** | `/api/rider/deliveries/available` |
| **인증** | Bearer Token |
| **권한** | RIDER |
...
| --- | --- |
| **Method** | `POST` |
| **URL** | `/api/rider/deliveries/{deliveryId}/accept` |
| **인증** | Bearer Token |
| **권한** | RIDER |
...
| --- | --- |
| **Method** | `PATCH` |
| **URL** | `/api/rider/deliveries/{deliveryId}/status` |
| **인증** | Bearer Token |
| **권한** | RIDER |
...
| --- | --- |
| **Method** | `POST` |
| **URL** | `/api/rider/deliveries/{deliveryId}/complete` |
| **인증** | Bearer Token |
| **권한** | RIDER |
...
| --- | --- |
| **Method** | `POST` |
| **URL** | `/api/rider/deliveries/{deliveryId}/message` |
| **인증** | Bearer Token |
| **권한** | RIDER |
...
| --- | --- |
| **Method** | `PATCH` |
| **URL** | `/api/subscriptions/{subscriptionId}/pause` |
| **인증** | Bearer Token |
| **권한** | CUSTOMER |
| **설명** | 활성 구독을 일시정지한다. |
| **관련 FR** | FR-SUB-003 |
...
| --- | --- |
| **Method** | `PATCH` |
| **URL** | `/api/subscriptions/{subscriptionId}/resume` |
| **인증** | Bearer Token |
| **권한** | CUSTOMER |
| **설명** | 일시정지된 구독을 재개한다. |
| **관련 FR** | FR-SUB-003 |
...
| --- | --- |
| **Method** | `POST` |
| **URL** | `/api/store/reviews/{reviewId}/reply` |
| **인증** | Bearer Token |
| **권한** | STORE_OWNER |
...
| --- | --- |
| **Method** | `PATCH` |
| **URL** | `/api/store/orders/{storeOrderId}/status` |
| **인증** | Bearer Token |
| **권한** | STORE_OWNER |
...
| --- | --- |
| **Method** | `PATCH` |
| **URL** | `/api/store/orders/{storeOrderId}/ready` |
| **인증** | Bearer Token |
| **권한** | STORE_OWNER |
...
| --- | --- |
| **Method** | `GET` |
| **URL** | `/api/store/subscription-products` |
| **인증** | Bearer Token |
| **권한** | STORE_OWNER |
...
| --- | --- |
| **Method** | `GET` |
| **URL** | `/api/store/subscriptions/subscribers` |
| **인증** | Bearer Token |
| **권한** | STORE_OWNER |
...
| --- | --- |
| **Method** | `PATCH` |
| **URL** | `/api/store/subscriptions/{subscriptionId}/status` |
| **인증** | Bearer Token |
| **권한** | STORE_OWNER |
...
| --- | --- |
| **Method** | `DELETE` |
| **URL** | `/api/rider/vehicles/{vehicleId}` |
| **인증** | Bearer Token |
| **권한** | RIDER |
...
| --- | --- |
| **Method** | `PATCH` |
| **URL** | `/api/admin/approvals/{approvalId}` |
| **인증** | Bearer Token |
| **권한** | ADMIN |
...
> 검증 결과: 28/29개 ENUM이 API에서 사용됨 ✅ (FR-DLV-009 주민배달원 관련 rider_approval_status는 riders 테이블의 approval status 필드로 API-ROP-008에서 활용)
> 
...
| --- | --- | --- | --- | --- | --- |
| 1 | Auth | AUTH-000 | GET | `/api/auth/check-email?email={email}` | Public |
| 2 | Auth | AUTH-000-2 | GET | `/api/auth/check-phone?phone={phone}` | Public |
| 3 | Auth | AUTH-001 | POST | `/api/auth/register` | Public |
| 4 | Auth | AUTH-002 | POST | `/api/auth/login` | Public |
| 5 | Auth | AUTH-003 | POST | `/api/auth/social-login` | Public |
| 6 | Auth | AUTH-004 | POST | `/api/auth/logout` | Bearer |
| 7 | Auth | AUTH-005 | POST | `/api/auth/send-verification` | Public |
| 8 | Auth | AUTH-006 | POST | `/api/auth/verify-phone` | Public |
| 9 | Auth | AUTH-007 | POST | `/api/auth/refresh` | Public |
| 10 | Users | USR-001 | GET | `/api/users/me` | Bearer |
| 11 | Users | USR-002 | PATCH | `/api/users/me` | Bearer |
| 12 | Users | USR-003 | DELETE | `/api/users/me` | Bearer |
| 13 | Users | USR-004 | GET | `/api/users/me/addresses` | Bearer |
| 14 | Users | USR-005 | POST | `/api/users/me/addresses` | Bearer |
| 15 | Users | USR-006 | PATCH | `/api/users/me/addresses/{addressId}` | Bearer |
| 16 | Users | USR-007 | DELETE | `/api/users/me/addresses/{addressId}` | Bearer |
| 17 | Users | USR-008 | GET | `/api/users/me/payment-methods` | Bearer |
| 18 | Users | USR-009 | POST | `/api/users/me/payment-methods` | Bearer |
| 19 | Users | USR-010 | DELETE | `/api/users/me/payment-methods/{id}` | Bearer |
| 20 | Stores | STO-001 | GET | `/api/stores` | Bearer |
| 21 | Stores | STO-002 | GET | `/api/stores/{storeId}` | Bearer |
| 22 | Stores | STO-003 | GET | `/api/stores/{storeId}/products` | Bearer |
| 23 | Stores | STO-004 | GET | `/api/stores/{storeId}/reviews` | Bearer |
| 24 | Stores | STO-005 | GET | `/api/stores/{storeId}/subscription-products` | Bearer |
| 25 | Stores | STO-006 | POST | `/api/stores/register` | Bearer |
| 26 | Products | PRD-001 | GET | `/api/products/search` | Bearer |
| 27 | Products | PRD-002 | GET | `/api/products/{productId}` | Bearer |
| 28 | Products | PRD-003 | GET | `/api/categories` | Public |
| 29 | Cart | CRT-001 | GET | `/api/cart` | Bearer |
| 30 | Cart | CRT-002 | POST | `/api/cart/items` | Bearer |
| 31 | Cart | CRT-003 | PATCH | `/api/cart/items/{cartItemId}` | Bearer |
| 32 | Cart | CRT-004 | DELETE | `/api/cart/items/{cartItemId}` | Bearer |
| 33 | Orders | ORD-001 | POST | `/api/orders` | Bearer |
| 34 | Orders | ORD-002 | GET | `/api/orders` | Bearer |
| 35 | Orders | ORD-003 | GET | `/api/orders/{orderId}` | Bearer |
| 36 | Orders | ORD-004 | POST | `/api/orders/{orderId}/cancel` | Bearer |
| 37 | Orders | ORD-005 | GET | `/api/orders/{orderId}/tracking` | Bearer |
| 38 | Orders | ORD-006 | POST | `/api/orders/{orderId}/reorder` | Bearer |
| 39 | Payments | PAY-001 | POST | `/api/payments` | Bearer |
| 40 | Payments | PAY-002 | GET | `/api/payments/{paymentId}` | Bearer |
| 41 | Payments | PAY-003 | POST | `/api/payments/{paymentId}/refund` | Bearer |
| 42 | Deliveries | DLV-001 | GET | `/api/rider/deliveries/available` | Bearer |
| 43 | Deliveries | DLV-002 | POST | `/api/rider/deliveries/{id}/accept` | Bearer |
| 44 | Deliveries | DLV-003 | PATCH | `/api/rider/deliveries/{id}/status` | Bearer |
| 45 | Deliveries | DLV-004 | POST | `/api/rider/deliveries/{id}/complete` | Bearer |
| 46 | Deliveries | DLV-005 | POST | `/api/rider/location` | Bearer |
| 47 | Deliveries | DLV-006 | PATCH | `/api/rider/status` | Bearer |
| 48 | Deliveries | DLV-007 | POST | `/api/rider/deliveries/{id}/message` | Bearer |
| 49 | Subscriptions | SUB-001 | POST | `/api/subscriptions` | Bearer |
| 50 | Subscriptions | SUB-002 | GET | `/api/subscriptions` | Bearer |
| 51 | Subscriptions | SUB-003 | PATCH | `/api/subscriptions/{id}/pause` | Bearer |
| 52 | Subscriptions | SUB-004 | PATCH | `/api/subscriptions/{id}/resume` | Bearer |
| 53 | Subscriptions | SUB-005 | DELETE | `/api/subscriptions/{id}` | Bearer |
| 54 | Reviews | REV-001 | POST | `/api/reviews` | Bearer |
| 55 | Reviews | REV-002 | PATCH | `/api/reviews/{reviewId}` | Bearer |
| 56 | Reviews | REV-003 | DELETE | `/api/reviews/{reviewId}` | Bearer |
| 57 | Reviews | REV-004 | POST | `/api/reviews/{reviewId}/report` | Bearer |
| 58 | Store Ops | SOP-001 | GET | `/api/store/dashboard` | Bearer |
| 59 | Store Ops | SOP-002 | GET | `/api/store/orders` | Bearer |
| 60 | Store Ops | SOP-003 | PATCH | `/api/store/orders/{id}/status` | Bearer |
| 61 | Store Ops | SOP-004 | PATCH | `/api/store/orders/{id}/ready` | Bearer |
| 62 | Store Ops | SOP-005 | POST | `/api/store/products` | Bearer |
| 63 | Store Ops | SOP-006 | PATCH | `/api/store/products/{id}` | Bearer |
| 64 | Store Ops | SOP-007 | DELETE | `/api/store/products/{id}` | Bearer |
| 65 | Store Ops | SOP-008 | PATCH | `/api/store/business-hours` | Bearer |
| 66 | Store Ops | SOP-009 | GET | `/api/store/subscription-products` | Bearer |
| 67 | Store Ops | SOP-010 | POST | `/api/store/subscription-products` | Bearer |
| 68 | Store Ops | SOP-011 | GET | `/api/store/settlements` | Bearer |
| 69 | Store Ops | SOP-012 | PATCH | `/api/store/active-status` | Bearer |
| 70 | Rider Ops | ROP-001 | GET | `/api/rider/dashboard` | Bearer |
| 71 | Rider Ops | ROP-002 | GET | `/api/rider/history` | Bearer |
| 72 | Rider Ops | ROP-003 | GET | `/api/rider/earnings` | Bearer |
| 73 | Rider Ops | ROP-004 | GET | `/api/rider/settlements` | Bearer |
| 74 | Rider Ops | ROP-005 | GET | `/api/rider/vehicles` | Bearer |
| 75 | Rider Ops | ROP-006 | POST | `/api/rider/vehicles` | Bearer |
| 76 | Rider Ops | ROP-007 | DELETE | `/api/rider/vehicles/{vehicleId}` | Bearer |
| 77 | Rider Ops | ROP-008 | POST | `/api/rider/register` | Bearer |
| 78 | Admin | ADM-001 | GET | `/api/admin/dashboard` | Bearer |
| 79 | Admin | ADM-002 | GET | `/api/admin/users` | Bearer |
| 80 | Admin | ADM-003 | PATCH | `/api/admin/users/{userId}/status` | Bearer |
| 81 | Admin | ADM-004 | GET | `/api/admin/stores` | Bearer |
| 82 | Admin | ADM-005 | PATCH | `/api/admin/stores/{storeId}/status` | Bearer |
| 83 | Admin | ADM-006 | GET | `/api/admin/riders` | Bearer |
| 84 | Admin | ADM-007 | GET | `/api/admin/approvals` | Bearer |
| 85 | Admin | ADM-008 | PATCH | `/api/admin/approvals/{id}` | Bearer |
| 86 | Admin | ADM-009 | GET | `/api/admin/reports` | Bearer |
| 87 | Admin | ADM-010 | PATCH | `/api/admin/reports/{reportId}` | Bearer |
| 88 | Admin | ADM-011 | GET | `/api/admin/settlements` | Bearer |
| 89 | Admin | ADM-012 | POST | `/api/admin/settlements` | Bearer |
| 90 | Admin | ADM-013 | GET | `/api/admin/notices` | Bearer |
| 91 | Admin | ADM-014 | POST | `/api/admin/notices` | Bearer |
| 92 | Admin | ADM-015 | PATCH | `/api/admin/notices/{noticeId}` | Bearer |
| 93 | Admin | ADM-016 | DELETE | `/api/admin/notices/{noticeId}` | Bearer |
| 94 | Admin | ADM-017 | POST | `/api/admin/notifications/broadcast` | Bearer |
| 95 | Misc | NTF-001 | GET | `/api/notifications` | Bearer |
| 96 | Misc | NTF-002 | PATCH | `/api/notifications/{id}/read` | Bearer |
| 97 | Misc | NTF-003 | PATCH | `/api/notifications/read-all` | Bearer |
| 98 | Misc | RPT-001 | POST | `/api/reports` | Bearer |
| 99 | Misc | INQ-001 | GET | `/api/inquiries` | Bearer |
| 100 | Misc | INQ-002 | POST | `/api/inquiries` | Bearer |
| 101 | Misc | NTC-001 | GET | `/api/notices` | Public |
| 102 | Misc | BNR-001 | GET | `/api/banners` | Public |
| 103 | Misc | PRM-001 | GET | `/api/promotions` | Public |
| 104 | Misc | CPN-001 | GET | `/api/coupons` | Bearer |
| 105 | Reviews | REV-005 | POST | `/api/store/reviews/{reviewId}/reply` | Bearer |
| 106 | Reviews | REV-006 | PATCH | `/api/store/reviews/{reviewId}/reply` | Bearer |
| 107 | Reviews | REV-007 | DELETE | `/api/store/reviews/{reviewId}/reply` | Bearer |
| 108 | Store Ops | SOP-010B | GET | `/api/store/subscriptions/subscribers` | Bearer |
| 109 | Store Ops | SOP-010C | PATCH | `/api/store/subscriptions/{id}/status` | Bearer |

> 총 REST API: 109개 | WebSocket 채널: 4개 | 총 113개 엔드포인트
> 
