 ...
 *   <li>deliveries.is_settled = true + settlement_id 벌크 UPDATE (JdbcTemplate)</li>
 *   <li>payment_refunds.is_settled = true 벌크 UPDATE — 해당 배달의 RIDER 귀책·APPROVED 환불만 (JdbcTemplate)</li>
 * </ol>
 *
 * <p>트랜잭션은 Spring Batch Step이 관리하므로 별도 {@code @Transactional} 불필요.</p>
 */
@Slf4j
...
public class RiderSettlementItemWriter implements ItemWriter<RiderSettlementDto> {
    ...
    public void write(Chunk<? extends RiderSettlementDto> chunk) {
        for (RiderSettlementDto dto : chunk) {
            int netAmount = Math.max(0, dto.getTotalEarning() - dto.getTotalRefund());
            ...
            log.info("[RiderSettlement] 정산 완료 — riderId={}, period={}~{}, totalEarning={}, totalRefund={}, netAmount={}",
